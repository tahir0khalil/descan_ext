...
NAME_ = 'r50_256_20241004'
# log_name = './Training_Loggger_text.txt'
log_name = './Encoder_logs_'+NAME_+'.txt'
...
class Custom_Loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_scan, clean_dist, img_clean
...
####
# color_encoder_path = '/home/tahir/workspace/descan_extension/AAAI_Github_Code_Descan/Train_color_encoder/weights/Color_encoder_r50_512FDL_001_20240919/30.pth'
# color_encoder_path = '/home/tahir/workspace/descan_extension/AAAI_Github_Code_Descan/Train_color_encoder/weights/Color_encoder_r50_256_2_20240912/50.pth'
# encoder_checkpoint = torch.load(color_encoder_path, map_location=device)
# modified_model.load_state_dict(encoder_checkpoint['model_state_dict']) 
####
...
criterion_2 = FDL_loss().cuda()
optimizer = optim.Adam(modified_model.parameters(),lr=0.0003,weight_decay = 0.00001)
...
def validate(test_folder, model_function, epoch):
    ...
    for x in  natsort.natsorted(listdir(join(test_folder, 'scan_512')), reverse=True):
        ...
        image_scan = Image.open(join(test_folder, 'scan_512', x))
        ...
        image_clean = Image.open(join(test_folder, 'clean_512', x))
...
def train_model(model, test_folder, criterion1, criterion2, optimizer, num_epochs):
    loss_train = []
    Epochs = []
    ...
    for epoch in range(num_epochs):
        ...
        for img_in, clean_dist, img_out in dataloaders:
            inputs = img_in.to(device)
            img_out = img_out.to(device)
            clean_dist = clean_dist.to(device)
            i += 1
            outputs = model(inputs)
            loss1 = criterion1(outputs, clean_dist) 
            ###
            mean_pred, std_pred = outputs[:,0:3], outputs[:, 3:]  
            mean_scan = torch.mean(inputs, dim=[2, 3])
            std_scan = torch.std(inputs, dim=[2, 3])
            normalized_scan = (inputs - mean_scan[:, :, None, None]) / (std_scan[:, :, None, None] + 1e-6)
            image_scan_shift = normalized_scan * std_pred[:, :, None, None] + mean_pred[:, :, None, None]
            image_scan_shift = image_scan_shift - image_scan_shift.min()
            image_scan_shift = image_scan_shift / image_scan_shift.max()
            loss2 = criterion2(img_out, image_scan_shift) 
            ### 
        ...
        if (epoch + 1) % 10 == 0:
            newpath = './weights/Color_encoder_'+NAME_
            if not os.path.exists(newpath):
                os.makedirs(newpath)
            torch.save({
                'epoch': epoch,
...
test_path = '/home/tahir/workspace/descan_extension/data_set/Descan_dataset/Valid'

model_trained = train_model(modified_model,test_path, criterion_1, criterion_2, optimizer, num_epochs=150)



# torch.save(model.state_dict(), 'Color_encoder_r50_256.h5')
